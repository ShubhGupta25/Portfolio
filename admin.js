...
const DEFAULT_SITE_DATA = (function () {
  return {
    meta: {
      name: "Shubh Gupta",
      tagline:
        "Java Developer | Microservices | Spring Boot | Cloud Enthusiast",
    },
    nav: [
...
function loadSaved() {
  try {
    ...
    if (!saved)
      return setStatus("No saved data found in this browser.", "muted");
...
function validate() {
  try {
    ...
    if (!parsed.meta || !parsed.nav)
      throw new Error("Missing required keys like meta/nav.");
...
function onFileChosen(e) {
  ...
  reader.onload = () => {
    ...
    setStatus(
      "Imported JSON (not saved yet). Click Validate -> Save.",
      "muted"
    );
  };
...
function updateAbout() {
  ...
  try {
    ...
    setStatus(
      "About section updated in editor (remember to Validate → Save).",
      "success"
    );
  } catch (e) {
...
function addRecommendation() {
  ...
  const tags = (els.qaRecTags.value || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  if (!name || !text)
    return setStatus("Name and Recommendation text are required.", "muted");
  try {
    const data = JSON.parse(els.editor.value || "{}");
    ...
    els.editor.value = JSON.stringify(data, null, 2);
    els.qaRecAvatar.value =
      els.qaRecName.value =
      els.qaRecTitle.value =
      els.qaRecText.value =
      els.qaRecTags.value =
        "";
...
function addCert() {
  ...
  const tags = (els.qaCertTags.value || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  if (!title || !issuer)
    return setStatus(
      "Title and Issuer are required for a certificate.",
      "muted"
    );

  try {
    const data = JSON.parse(els.editor.value);
    data.certificates = Array.isArray(data.certificates)
      ? data.certificates
      : [];
    data.certificates.push({ title, issuer, meta, verify, img, tags });
    els.editor.value = JSON.stringify(data, null, 2);
    els.qaCertTitle.value =
      els.qaCertIssuer.value =
      els.qaCertMeta.value =
      els.qaCertUrl.value =
      els.qaCertImg.value =
      els.qaCertTags.value =
        "";
